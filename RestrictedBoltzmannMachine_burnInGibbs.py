...
class RBM:
    ...
    def train(self, data, max_epochs=1000, learning_rate=0.1, m=1000, k=5, burn_in_drop=1, num_drop=100):
...
if __name__ == '__main__':
    ...
    imgs[mnist > 127] = 1
    max_epochs = 1000
    error_list_100 = r.train(imgs, max_epochs, burn_in_drop=1)
    error_list_1000 = r.train(imgs, max_epochs, burn_in_drop=1, num_drop=1000)

    # Plotting
    #error_list = [x for x in error_list]
    epochs = np.arange(max_epochs)
    plt.title('Error values with modified length of burn-in phase')
    plt.xlabel('Number of epochs')
    plt.ylabel('Error values')
    plt.xticks(np.arange(max_epochs, step=50))
    plt.plot(epochs, error_list_100, 'r', label='100 states dropped out')
    plt.plot(epochs, error_list_1000, 'b', label='1000 states dropped out')
    plt.legend()
    plt.savefig('plot_burnIn.png')
